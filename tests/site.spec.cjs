...
test('Home notice follows the nearest scheduled event and hides with an empty calendar',async({page})=>{
 ...
 const note=page.locator('#next-event-note');await expect(note).toBeVisible();await expect(note.locator('h2')).toHaveText('First event');await expect(note.locator('.next-event-note__status')).toHaveText('Entradas disponibles');await expect(note.locator('a')).toHaveAttribute('href','events-upcoming.html');
 await page.evaluate(()=>window.LSVZ_I18N.setLang('de'));await expect(note.locator('.next-event-note__status')).toHaveText('Tickets verfügbar');
 ...
 await page.reload();await page.evaluate(()=>document.body.classList.add('intro-done'));await expect(note).toBeVisible();await expect(note.locator('h2')).toHaveText('New event');await expect(note.locator('.next-event-note__status')).toHaveText('Eintritt frei');
